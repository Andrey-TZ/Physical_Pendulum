...
class DryFriction:
    ...
    def count(self):
        ...
        while j < 100:
            ...
            k0 = self.h * (-self.k * np.sign(self.speed[i]) - np.sin(self.angle[i]))
            c1 = self.h * (self.speed[i] + 0.5 * k0)
            k1 = self.h * (-self.k * np.sign(self.speed[i]) - np.sin(self.angle[i] + 0.5 * c0))
            c2 = self.h * (self.speed[i] + 0.5 * k1)
            k2 = self.h * (-self.k * np.sign(self.speed[i]) - np.sin(self.angle[i] + 0.5 * c1))
            c3 = self.h * (self.speed[i] + k2)
            k3 = self.h * (-self.k * np.sign(self.speed[i]) - np.sin(self.angle[i] + c2))
